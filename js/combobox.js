$(document).ready(function() {
    // Khởi tạo sự kiện
    $('.d-combobox-button').click(btnComboboxOnClick);
    // $('.d-combobox-contain .d-combobox-item').click(itemComboboxOnClick);
    $('.d-combobox-contain').on('click', '.d-combobox-item', itemComboboxOnClick);

    // // Lưu trữ thông tin combobox data:
    // let comboboxs = $('.d-combobox-contain');
    // for (const combobox of comboboxs) {
    //     let itemDataElements = $(combobox).find('.d-combobox-data').html();
    //     $(combobox).data("itemDataElement", itemDataElements);
    //     $(combobox).find('.d-combobox-data').empty();
    // }
    $(".d-combobox-contain input").keydown(inputComboboxOnKeyDown);
    $(".d-combobox-contain input").keyup(inputComboboxOnKeyUp);
...
function inputComboboxOnKeyUp() {
    ...
    switch (event.keyCode) {
        ...
        default:
            ...
            let items = $(this).siblings('.d-combobox-data').children();
...
function inputComboboxOnKeyDown() {
    ...
    // $(items).removeClass("d-combobox-item-focus");
    ...
    switch (event.keyCode) {
        case 13: // Khi nhấn phím Enter:
            ...
            if (itemHoverred.length == 1) {
                ...
                let parentComboboxElement = $(itemHoverred).parents('.d-combobox-contain');
...
function btnComboboxOnClick() {
    $(this).siblings('.d-combobox-data').toggle();
}
...
function itemComboboxOnClick() {
    ...
    //Assign value for combobox
    let parentCbbElement = $('this').parent('d-combobox-contain');
    parentCbbElement.attr('value', value);
    // parentCbbElement.data('value', value);
    // Hide items cbb
    $(parentElement).hide()
}